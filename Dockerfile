...
RUN wget -O - https://apt.llvm.org/llvm-snapshot.gpg.key | apt-key add - && \
    ...
    apt-get update && \
    apt-get install -y clang-9 llvm-9-dev llvm-9 llvm-9-tools llvm-9-runtime libboost1.68-dev
...
WORKDIR $GAZER_DIR
RUN cmake -DGAZER_ENABLE_UNIT_TESTS=On -DCMAKE_BUILD_TYPE=Debug -DCMAKE_EXPORT_COMPILE_COMMANDS=On . && make
