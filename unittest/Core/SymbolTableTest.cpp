...
TEST(SymbolTable, CanCreateVariables)
{
    ...
    ASSERT_EQ(x.getType(), *IntType::get());
...
{
    SymbolTable st;
    st.create("x", IntType::get());

    EXPECT_THROW(st.create("x", IntType::get()), gazer::SymbolAlreadyExistsError);
...
{
    SymbolTable st;
    auto& x = st.create("x", IntType::get());
